...
class TaxIdSpec extends FunSuite {
  ...
  test("TaxId.fromString tries CPF first, then CNPJ") {
    // Valid CPF format
    val cpfResult = TaxId.fromString("123.456.789-09")
    assert(cpfResult.isRight, "Should parse valid CPF")
    cpfResult.foreach { taxId =>
      assert(taxId.isCPF, "Should be identified as CPF")
    }

    // Valid CNPJ format
    val cnpjResult = TaxId.fromString("11.222.333/0001-81")
    assert(cnpjResult.isRight, "Should parse valid CNPJ")
    cnpjResult.foreach { taxId =>
      assert(taxId.isCNPJ, "Should be identified as CNPJ")
    }
  }

  test("TaxId.fromString fails for invalid input") {
    val result = TaxId.fromString("invalid-tax-id")
    assert(result.isLeft, "Should fail for invalid input")
