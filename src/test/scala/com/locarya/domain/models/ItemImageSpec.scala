...
class ItemImageSpec extends FunSuite:
  ...
  test("create with single URL produces one image that is primary at displayOrder 1") {
    val result = ItemImage.create(itemId, List(url1))
    ...
    result.foreach { images =>
      ...
      assertEquals(images.head.itemId, itemId)
    }
  }

  test("create with multiple URLs sets only first as primary") {
    val result = ItemImage.create(itemId, List(url1, url2, url3))
    assert(result.isRight, s"Expected Right but got $result")
    result.foreach { images =>
  ...
  test("create assigns distinct ItemImageIds") {
    val result = ItemImage.create(itemId, List(url1, url2))
    assert(result.isRight)
    result.foreach { images =>
  ...
  test("create with empty list fails with InvalidItemImage") {
    val result = ItemImage.create(itemId, List.empty)
    assert(result.isLeft, "Expected Left for empty URL list")
    result.left.foreach { err =>
      assert(err.isInstanceOf[InvalidItemImage], s"Expected InvalidItemImage but got $err")
    }
  }

  test("create with more than 5 URLs fails with InvalidItemImage") {
    val urls = (1 to 6).map(i => s"https://example.com/img$i.jpg").toList
    val result = ItemImage.create(itemId, urls)
  ...
  test("create with an invalid URL fails with validation error") {
    val result = ItemImage.create(itemId, List("not-a-url"))
